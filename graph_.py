...
class Edge:
    ...
    def __str__(self) -> str:
        return (
            f'{self.source} -|{self.data["label"]}|-> {self.target} [weight={self.data["weight"]}]'
            if self.data["label"]
            else f'{self.source} -> {self.target} [weight={self.data["weight"]}]'
        )
...
class Graph:
    ...
    def add_edge(self, source: str, target: str, label="", weight: float = 1):
        ...
        self.edges.update(
            {
                (source, target): Edge(
                    self.nodes[source], self.nodes[target], label, weight
                )
            }
        )
    ...
    def save(self, filename):
        with open(f"{filename}_edges.csv", "w") as f:
            ...
            for edge in self.edges.values():
                f.write(
                    f'{edge.source.name},{edge.target.name},{edge.data["label"]},{edge.data["weight"]}\n'
                )
    ...
    def load(filename) -> "Graph":
        ...
        with open(f"{filename}_edges.csv") as f:
            ...
            for row in reader:
                g.add_edge(
                    row["source"], row["target"], row["label"], float(row["weight"])
                )
